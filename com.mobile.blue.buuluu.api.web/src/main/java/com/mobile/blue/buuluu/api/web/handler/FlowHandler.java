...
import com.mobile.blue.buuluu.api.common.exception.SourceTypeException;
import com.mobile.blue.buuluu.api.common.util.Constant;
import com.mobile.blue.buuluu.api.common.util.PropertiesUtil;
import com.mobile.blue.buuluu.api.model.DownLoadAdsModel;
import com.mobile.blue.buuluu.api.model.DownLoadAppModel;
...
//		return null;
	...
	public List getAppsList2(String userId, Integer type,Integer pageNo) {
		List<RandomAdsModel> modelList = new ArrayList<RandomAdsModel>();
			if(pageNo==null|| pageNo<=1)
				pageNo=1;
				List<Map<String, Object>> list2 = sourceService.getAppList(1, -1, "createTime", "desc", 1,null);
	...
	private List<RandomAdsModel> getVideoList2(List<Map<String, Object>> list){
		List<RandomAdsModel> modelList = new ArrayList<RandomAdsModel>();
		for (Map<String, Object> map : list) {
			RandomAdsModel model = new RandomAdsModel();
			model.setIcon(map.get("picture").toString());
			model.setMakeFlows(Float.parseFloat(map.get("flowCoins").toString()));
			model.setSourceId(Integer.parseInt(map.get("id").toString()));
